...
class ControllerExtensionPaymentPaysafecash extends Controller
{
    ...
    public function index()
    {
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/paysafecash_redirect')) {
            return $this->load->view($this->config->get('config_template') . '/template/extension/payment/paysafecash_redirect', $data);
        } else {
            return $this->load->view('extension/payment/paysafecash_redirect', $data);
        }
    }
    ...
    public function redirect()
    {
        ...
        if (isset($response["object"])) {
            ...
            $this->model_extension_payment_paysafecash->validatePaymentOrder($order_info["order_id"], (int)$this->config->get('paysafecash_awaiting_order_status_id'), $response["id"]);
    ...
    public function failure()
    {
        ...
        if ($order_id > 0) {
            $order_info = $this->model_checkout_order->getOrder($order_id);
            $this->model_extension_payment_paysafecash->validatePaymentOrder($order_info["order_id"], (int)$this->config->get('paysafecash_declined_order_status_id'), $payment_id);
    ...
    public function webhook()
    {
        ...
        if ($status) {
            ...
            if ($order_id > 0 && $payment_id) {
                $check_order_data = $this->model_extension_payment_paysafecash->getPaymentAndOrder($order_id, $payment_id);
    ...
    private function completeOrder($order, $payment_id, $code)
    {
        ...
        if ($order['order_status_id'] == (int)$this->config->get('paysafecash_awaiting_order_status_id')) {
            $this->model_extension_payment_paysafecash->validatePaymentOrder($order['order_id'], (int)$this->config->get('paysafecash_order_status_id'), $payment_id);
