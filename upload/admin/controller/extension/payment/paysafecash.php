...
class ControllerExtensionPaymentPaysafecash extends Controller
{
    ...
    private $version_url = 'https://raw.githubusercontent.com/IndiAsh/Version-Checker/master/version.json';

    public function index()
    {
        $this->load->language('extension/payment/paysafecash');
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            if (version_compare(VERSION, '3.0.0.0', '<')) {
                ...
                $this->model_setting_setting->editSetting('payment_paysafecash', $this->request->post);
                $this->session->data['success'] = $this->language->get('text_success');
        ...
        if (version_compare(VERSION, '3.0.0.0', '<')) {
            $data['link_check_version'] = $this->url->link('extension/payment/paysafecash/checkversion'.'&token=' . $this->session->data['token'], '', true);
            $data['breadcrumbs'] = [];
            $data['breadcrumbs'][] = [
            ...
            $data['breadcrumbs'][] = [
                ...
                'href' => $this->url->link('extension/payment/paysafecash', 'token=' . $this->session->data['token'], true)
            ];
            $data['action'] = $this->url->link('extension/payment/paysafecash', 'token=' . $this->session->data['token'], true);
            $data['cancel'] = $this->url->link('extension/extension', 'token=' . $this->session->data['token'] . '&type=payment', true);
        } else {
            $data['link_check_version'] = $this->url->link('extension/payment/paysafecash/checkversion'.'&user_token=' . $this->session->data['user_token'], '', true);
            $data['breadcrumbs'] = [];
            $data['breadcrumbs'][] = [
                'text' => $this->language->get('text_home'),
            ...
            $data['breadcrumbs'][] = [
                'text' => $this->language->get('heading_title'),
                'href' => $this->url->link('extension/payment/paysafecash', 'user_token=' . $this->session->data['user_token'], true)
            ];
            $data['action'] = $this->url->link('extension/payment/paysafecash', 'user_token=' . $this->session->data['user_token'], true);
        ...
        if (version_compare(VERSION, '3.0.0.0', '<')) {
            ...
            if (isset($this->request->post['payment_paysafecash_status'])) {
                $data['payment_paysafecash_status'] = $this->request->post['payment_paysafecash_status'];
            } else {
                $data['payment_paysafecash_status'] = $this->config->get('payment_paysafecash_status');
            }

            if (isset($this->request->post['payment_paysafecash_sort_order'])) {
                $data['payment_paysafecash_sort_order'] = $this->request->post['payment_paysafecash_sort_order'];
            } else {
                $data['payment_paysafecash_sort_order'] = $this->config->get('payment_paysafecash_sort_order');
            }
        }
        ...
        $this->response->setOutput($this->load->view('extension/payment/paysafecash', $data));
    }

    private function validate()
    {
        if (!$this->user->hasPermission('modify', 'extension/payment/paysafecash')) {
    ...
    public function listorders()
    {
        ...
        if (version_compare(VERSION, '3.0.0.0', '<')) {
            ...
            $data['breadcrumbs'][] = [
                'text' => $this->language->get('heading_title'),
                'href' => $this->url->link('extension/payment/paysafecash/listorders', 'token=' . $this->session->data['token'] . $url, true)
            ];
        } else {
            $data['breadcrumbs'][] = [
                'text' => $this->language->get('text_home'),
            ...
            $data['breadcrumbs'][] = [
                'text' => $this->language->get('heading_title'),
                'href' => $this->url->link('extension/payment/paysafecash/listorders', 'user_token=' . $this->session->data['user_token'] . $url, true)
            ];
        }
        ...
        $check_installation = $this->model_extension_payment_paysafecash->checkInstall();
        if (!$check_installation) {
            ...
            if (version_compare(VERSION, '3.0.0.0', '<')) {
                $data['refund'] = $this->url->link('extension/payment/paysafecash/refund', 'token=' . $this->session->data['token'] . $url, true);
            } else {
                $data['refund'] = $this->url->link('extension/payment/paysafecash/refund', 'user_token=' . $this->session->data['user_token'] . $url, true);
            }
            ...
            $results = $this->model_extension_payment_paysafecash->listorders($filter_data);
            $totalorders = $this->model_extension_payment_paysafecash->countorders($filter_data);
            ...
            if (version_compare(VERSION, '3.0.0.0', '<')) {
                $data['sort_order'] = $this->url->link('extension/payment/paysafecash/listorders', 'token=' . $this->session->data['token'] . '&sort=order_id' . $url, true);
                $data['sort_status'] = $this->url->link('extension/payment/paysafecash/listorders', 'token=' . $this->session->data['token'] . '&sort=status' . $url, true);
            } else {
                $data['sort_order'] = $this->url->link('extension/payment/paysafecash/listorders', 'user_token=' . $this->session->data['user_token'] . '&sort=order_id' . $url, true);
                $data['sort_status'] = $this->url->link('extension/payment/paysafecash/listorders', 'user_token=' . $this->session->data['user_token'] . '&sort=status' . $url, true);
            }
            ...
            if (version_compare(VERSION, '3.0.0.0', '<')) {
                $pagination->url = $this->url->link('extension/payment/paysafecash/listorders', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
            } else {
                $pagination->url = $this->url->link('extension/payment/paysafecash/listorders', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
            }
        ...
        $data['column_left'] = $this->load->controller('common/column_left');
        $data['footer'] = $this->load->controller('common/footer');
        $this->response->setOutput($this->load->view('extension/payment/paysafecash_orders', $data));
    }
    ...
    public function refund()
    {
        ...
        foreach ($this->request->post['selected_orders'] as $order) {
            if ($order > 0) {
                $order_info = $this->model_extension_payment_paysafecash->listorders(['filter_order_id' => $order]);
                ...
                if (count($order_info)) {
                    ...
                    if ($response == false || isset($response['number'])) {
                        ...
                        if ($response["status"] == "SUCCESS") {
                            $this->model_extension_payment_paysafecash->doRefund($order, $order_info[0]['payment_id'], ['ref_id' => $response["id"], 'status' => (int)$this->config->get('paysafecash_refund_order_status_id')]);
    ...
    {
        $this->load->model('extension/payment/paysafecash');
        $this->model_extension_payment_paysafecash->install();
    }
    ...
    {
        $this->load->model('extension/payment/paysafecash');
        $this->model_extension_payment_paysafecash->uninstall();
    }
        ...
        if ($data) {
            echo json_encode([
                ...
                'data' => [
                    ...
                    'latest_version' => $data['oc23']['current_version'],
                    'changelog' => $data['oc23']['changelog'],
                    'lastupdate' => $data['oc23']['last_update'],
                ],
            ]);
        } else {
